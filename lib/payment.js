...
Payment.prototype._signedQuery = function* _signedQuery(url, params, options){
  ...
  if(missing.length){
    return throw new Error('missing params ' + missing.join(','));
  }
...
Payment.prototype.orderQuery = function* orderQuery(params){
...
Payment.prototype.middleware = function middleware() {
  ...
  function fail(err) {
    ...
    this.body = self.buildXml({
      return_code: 'FAIL',
      return_msg: err.message || err.name || ''df
    });
  }
